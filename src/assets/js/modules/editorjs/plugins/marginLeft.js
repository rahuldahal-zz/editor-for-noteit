export default class MarginLeft {
  static get isInline() {
    return true;
  }
  ...
  surround(range) {
    ...
    if (parentElement.classList.contains("ml1rem")) {
      this.unwrap(range);
  ...
  unwrap(range) {
    ...
    if (selectedTag.classList.length === 1) {
      ...
      selectedTag.classList.remove("ml1rem");
    }
  }

  checkState(selection) {
    let termWrapper = this.api.selection.findParentTag(this.tag, "ml1rem");
